...
def resolve_backend_for_model(model: str) -> LLMBackend:
    ...
    if "claude" in lowered:
        ...
        return AnthropicLLMBackend(model=model, api_key=config.anthropic_key, base_url=config.anthropic_base_url)

    if lowered.startswith(("gpt-", "o1-")) or "glm" in lowered:
        if not config.openai_key:
            raise ConfigurationError("OPENAI_API_KEY must be set.")
        return OpenAILLMBackend(model=model, api_key=config.openai_key, base_url=config.openai_base_url, organization=config.openai_org)

    if "gemini" in lowered or lowered.startswith("g-"):
        if not config.google_key:
            raise ConfigurationError("GOOGLE_API_KEY must be set.")
        return GoogleLLMBackend(model=model, api_key=config.google_key, base_url=config.google_base_url)
...
def get_backend_from_env() -> LLMBackend:
    ...
    if config.provider in {"anthropic", "auto"} and config.anthropic_key:
        return AnthropicLLMBackend(model=config.model, api_key=config.anthropic_key, base_url=config.anthropic_base_url)

    if config.provider in {"openai", "auto"} and config.openai_key:
        return OpenAILLMBackend(model=config.model, api_key=config.openai_key, base_url=config.openai_base_url, organization=config.openai_org)

    if config.provider in {"google", "auto"} and config.google_key:
        return GoogleLLMBackend(model=config.model, api_key=config.google_key, base_url=config.google_base_url)
    ...
    if config.provider == "custom_http":
        ...
        return CustomHTTPLLMBackend(model=config.model, api_base_url=config.custom_base_url, api_key=config.custom_api_key, timeout=config.custom_timeout)
